# Create React App (TypeScript) + Stacks.js
...
- **`src/components/ConnectWallet.tsx`** — connecting a wallet with `connect()` and reading the
  connected STX address from `getLocalStorage()`, plus `disconnect()` / `isConnected()`.
- **`src/components/ContractCallVote.tsx`** — a testnet contract call via
